...
def ConnectMaps(selection=False):
    ...
    for shader in shaders :
        ...
        if mc.objExists(scene_name + '_' + shader_name + '_BaseColor'): # Check input BaseColor if empty connect file node
            file_BaseColor = scene_name + '_' + shader_name + '_BaseColor'
            ...
            upstream_BaseColor = mc.listConnections(shader + '.baseColor', d= False, s=True)
            if upstream_BaseColor ==  None :
                file_BaseColor = mc.shadingNode('file', asTexture=True, isColorManaged=True, name= scene_name + '_' + shader_name + '_BaseColor')
                mc.connectAttr( file_BaseColor + '.outColor', shader + '.baseColor')
            else: 
                pass

        if mc.objExists(scene_name + '_' + shader_name + '_Height'): # Check input Height if empty connect file node
            ...
            upstream_Height = mc.listConnections(shading_group + '.displacementShader', d= False, s=True)
            if upstream_Height ==  None :
                file_Height = mc.shadingNode('file', asTexture=True, isColorManaged=True, name= scene_name + '_' + shader_name + '_Height')
        ...
        if mc.objExists(scene_name + '_' + shader_name + '_Metalness'): # Check input Metalness if empty connect file node
            ...
            upstream_Metalness = mc.listConnections(shader + '.metalness', d= False, s=True)
            if upstream_Metalness ==  None :
                file_Metalness = mc.shadingNode('file', asTexture=True, isColorManaged=True, name= scene_name + '_' + shader_name + '_Metalness')
                mc.setAttr(file_Metalness + '.alphaIsLuminance', 1)
                mc.connectAttr( file_Metalness + '.outAlpha', shader + '.metalness')
            else: 
                pass

        if mc.objExists(scene_name + '_' + shader_name + '_Roughness'): # Check input Roughness if empty connect file node
            ...
            upstream_Roughness = mc.listConnections(shader + '.specularRoughness', d= False, s=True)
            if upstream_Roughness ==  None :
                file_Roughness = mc.shadingNode('file', asTexture=True, isColorManaged=True, name= scene_name + '_' + shader_name + '_Roughness')
                mc.setAttr(file_Roughness + '.alphaIsLuminance', 1)
                mc.connectAttr( file_Roughness + '.outAlpha', shader + '.specularRoughness')
            else: 
                pass

        if mc.objExists(scene_name + '_' + shader_name + '_Normal'): # Check input Normal if empty connect file node
            ...
            upstream_Normal = mc.listConnections(shader + '.normalCamera', d= False, s=True)
            if upstream_Normal ==  None :
                file_Normal = mc.shadingNode('file', asTexture=True, isColorManaged=True, name= scene_name + '_' + shader_name + '_Normal')
                normal_node = core.createArnoldNode('aiNormalMap', name = scene_name + '_aiNormal_' + shader_name)
                mc.connectAttr( file_Normal + '.outColor', normal_node + '.input')
                mc.connectAttr( normal_node + '.outValue', shader + '.normalCamera')
            else: 
